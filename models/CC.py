...
import numpy as np
from misc.FocalL2 import Focal_L2
...
class CrowdCounter(nn.Module):
    def __init__(self, net_name, gpu_id):
        ...
        elif cfg.LOSS_FUNCTION == 'FocalL2_loss':
            self.loss_focalL2_fn = Focal_L2(0.1).cuda()
            self.loss_cossim_fn = CosSim_Loss().cuda()
        else:
            return 'error'
    ...
    def forward(self, data, num_class, iter = 0,  mode = 'train'):
        ...
        if mode == 'train':
            ...
            if cfg.LOSS_FUNCTION == 'Cos_loss':
                ...
                self.loss_fn = self.build_cos_loss(pred_map, gauss_map, iter, alpha=self.alpha, beta=1, lambda_=0.0001, reg_params=self.CCN.parameters())
                ...
                gauss_map = self.multi_class_gauss_map_generate(gt_map)
                self.loss_fn = self.build_focalL2_loss(pred_map, gauss_map, iter, alpha=self.alpha, beta=1, lambda_=0.0001, reg_params=self.CCN.parameters())
                outputs['gauss_map'] = gauss_map
                return outputs
    ...
    def build_cos_loss(self, pred_map, gauss_map, iter, alpha, beta, lambda_, reg_params):
        ...
        loss = lambda_ * c_loss + beta * mse_loss
    ...
    def build_focalL2_loss(self, pred_map, gauss_map, iter, alpha, beta, lambda_, reg_params):
        f_loss = self.loss_focalL2_fn(pred_map, gauss_map)
        c_loss = self.loss_cossim_fn(pred_map).cuda()

        loss = lambda_ * c_loss + beta * f_loss

        if (iter + 1) % 20 == 0:
            print('focalL2_loss:', f_loss)
    ...
    def build_mask_loss(self, pred_map, gauss_map, mask_map, pred_mask_maps, iter):  
        ...
        for c_idx in range(mask_map.shape[1]):
            seg_loss = seg_loss + self.loss_seg_fn(pred_mask_maps[c_idx], mask_map[:,c_idx,:,:].long())
