...
class SAM(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = F.softmax(energy, dim=-1).clone()
...
class GatingBlock(nn.Module):
    ...
    def __init__(self, num_features, output_num):
        ...
        self.conv1 = Conv2d1(num_features, 256, 3, same_padding=True)
        self.conv2 = Conv2d1(256, output_num, 1, same_padding=True)
...
class MOCN(nn.Module):
    ...
    def __init__(self, num_class=6,  pretrained=True):
        ...
        self.gating1 = GatingBlock(512, 3)
    ...
    def forward(self,rgb):
        ...
        oldx = x[2]
        x = self.neck(x)
        new_x0 = F.interpolate(x[0], scale_factor=0.5)
        new_x2 = F.interpolate(x[2], scale_factor=2)
        wmaps_ = self.gating1(oldx, new_x0.shape)
        wmaps1 = wmaps_[0]
        wmaps2 = wmaps_[1]
        wmaps3 = wmaps_[2]
        new_x0 = new_x0 * wmaps1
        new_x1 = x[1] * wmaps2
        new_x2 = new_x2 * wmaps3
        x = torch.cat([new_x0, new_x1, new_x2], dim=1)
        x = self.reduce(x)
