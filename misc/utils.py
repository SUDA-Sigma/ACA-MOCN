...
def eval_game(output, target, L=0):
    ...
    for i in range(p):
        for j in range(p):
            ...
            abs_error = abs_error + abs(output_block.sum()-target_block.sum().float())
            square_error = square_error + (output_block.sum()-target_block.sum().float()).pow(2)
...
def logger(exp_path, exp_name, work_dir, exception, resume=False):
    ...
    cfg_file = open('./config.py', "r", encoding="utf-8")
...
class AveragePosNegMeter(object):
    ...
    def update(self, cur_val, pos, class_id):
        ...
        if pos == 1:          #正样本
            self.pos_count[class_id] = self.pos_count[class_id] + 1
            self.pos_sum[class_id] = self.pos_sum[class_id] + cur_val
        else:
            self.neg_count[class_id] = self.neg_count[class_id] + 1
            self.neg_sum[class_id] = self.neg_sum[class_id] + cur_val
...
class AverageCategoryMeter(object):
    ...
    def update(self, cur_val, class_id):
        self.cur_val[class_id] = cur_val
        self.sum[class_id] = self.sum[class_id] + cur_val
        self.count[class_id] = self.count[class_id] + 1
...
class Timer(object):
    ...
    def toc(self, average=True):
        ...
        self.total_time = self.total_time + self.diff
